...
function render(){
 renderRewards();const ready=tasks.filter(t=>t.status==='ready'),done=tasks.filter(t=>t.status==='done'),pending=tasks.filter(t=>t.status==='scored');
 $('#total').textContent=String(rewards.lifetime).padStart(4,'0');$('#done-count').textContent=done.length;$('#task-count').textContent=ready.length;$('#balls').textContent=ready.length;$('.led').textContent=rolling?'● BALL IN PLAY':ready.length?'● READY TO PLAY':'○ LOAD A TASK';
 ...
 const rack=$('#tasks'),scroll=rack.scrollTop;rack.replaceChildren();if(!ready.length)rack.append(el('p','rack-empty','No tasks waiting to roll.'));for(const t of ready){const row=el('div','task-title-row',t.title);row.setAttribute('role','listitem');rack.append(row);}rack.scrollTop=scroll;
...
$('#test-logo').addEventListener('dblclick',()=>{rewards.wallet+=1000;rewards.lifetime+=1000;save();render();toast('+1,000 test points');});
$('#add-task').onclick=()=>{$('#task-dialog').showModal();$('#task-title').focus();};$('#close-dialog').onclick=()=>$('#task-dialog').close();$('#task-form').onsubmit=e=>{e.preventDefault();try{addTask($('#task-title').value,$('#task-note').value);$('#task-form').reset();$('#task-dialog').close();toast('Ball added.');}catch(error){toast(error.message);}};let helpClickTimer;$('#help').onclick=event=>{clearTimeout(helpClickTimer);if(event.detail===0){$('#help-dialog').showModal();return;}if(event.detail===1)helpClickTimer=setTimeout(()=>$('#help-dialog').showModal(),500);};$('#help').ondblclick=event=>{event.preventDefault();clearTimeout(helpClickTimer);if($('#help-dialog').open)$('#help-dialog').close();rewards.wallet+=1000;rewards.lifetime+=1000;save();render();toast('+1,000 test points');};$('#close-help').onclick=()=>$('#help-dialog').close();document.querySelectorAll('dialog').forEach(d=>d.addEventListener('click',e=>{if(e.target===d){const r=d.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)d.close();}}));
...
// Keep the header decorative: only the logo's deliberate double-click test hook remains.
